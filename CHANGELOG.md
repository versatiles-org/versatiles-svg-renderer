...
- **`line-gap-width`.** A line with a gap (typically a road casing) was drawn as one solid line. It is now drawn as in MapLibre GL JS: two lines of `line-width`, one on each side of the gap. Unlike MapLibre, which extrudes one band from the line, the two sides are separate lines: round caps at the line's ends do not join them, and at sharp turns with round joins their corners stay pointed. ([#53](https://github.com/versatiles-org/versatiles-svg-renderer/issues/53))
