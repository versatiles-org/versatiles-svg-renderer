...
- **`line-pattern`.** Lines with a pattern were drawn in their plain color. They now repeat the sprite image along the line as in MapLibre GL JS: its height spans the line's width, one copy is as long as the image keeps its aspect ratio at the line's width at the zoom level's integer part, and the copies grow with the map up to the fractional zoom. The image's top is on the right of the line's direction, and a polygon's outline is measured from its last vertex, as in MapLibre. Joins and caps are cut as the line is stroked; where MapLibre bends the image around a sharp join, the renderers continue it straight. A missing image draws nothing; `line-opacity` and `line-translate` apply, `line-dasharray` and `line-blur` do not. ([#44](https://github.com/versatiles-org/versatiles-svg-renderer/issues/44))
- **GeoJSON source options `filter`, `promoteId` and `generateId`.** A GeoJSON source's `filter` was ignored, and so were the ids these options give features. The filter now keeps only the features that pass it, evaluated at zoom 0 as in MapLibre GL JS; an invalid filter leaves the source empty and is reported through `onWarning`. `promoteId` and `generateId` set the id that `['id']` reads, and ids are read as MapLibre reads them: a string id as an integer. As in MapLibre, paint properties see a promoted id as the property is, while filters and layout properties see it as an integer; this also applies to `promoteId` on vector sources. ([#52](https://github.com/versatiles-org/versatiles-svg-renderer/issues/52))
- **`image` sources.** A raster layer on an `image` source drew nothing. The image is now loaded (with the `fetch` option, cached like a tile) and placed on its four `coordinates` as in MapLibre GL JS: a lon/lat rectangle as it is, any other quad warped as MapLibre warps it, in perspective, blended towards a bilinear warp where it is strongly foreshortened. It turns with a bearing and bends over the globe, and the `raster-*` paint properties apply. `video` sources remain unsupported. ([#51](https://github.com/versatiles-org/versatiles-svg-renderer/issues/51))
- **`icon-text-fit` and stretchable icons.** Icons that a style fits to their labels, such as road shields, were drawn at their own size. They now span their label as in MapLibre GL JS: its width, height or both (`icon-text-fit`), plus `icon-text-fit-padding`, centered on the label, and scaled by `icon-size`. Sprite images with stretch zones (`stretchX`, `stretchY`) stretch only there, keeping their corners and edges; `content` sets the part fitted to the label, and `textFitWidth` and `textFitHeight` keep its aspect ratio. Unlike MapLibre, a stretched icon turned by `icon-rotate` keeps its corners in place. ([#54](https://github.com/versatiles-org/versatiles-svg-renderer/issues/54))
...
- **Labels of several lines.** A label was always one line. Point labels are now broken into lines as MapLibre GL JS breaks them: at most `text-max-width` ems wide (10 by default, so this affects styles that do not set it), at spaces, hyphens and other break points or between CJK characters, with lines of about equal width, and always at a newline. `text-line-height`, `text-justify` and `text-letter-spacing` are supported. Labels along lines stay one line, as in MapLibre. ([#41](https://github.com/versatiles-org/versatiles-svg-renderer/issues/41))
- **Labels along lines.** Labels with `symbol-placement: "line"` or `"line-center"`, typically street and river names, were drawn as straight text. They now follow their lines as in MapLibre GL JS: repeated every `symbol-spacing` where the whole label fits, left out where the line bends more than `text-max-angle`, laid out glyph by glyph along the line and kept upright (`text-keep-upright`). Icons along lines, such as one-way arrows, turn with the line when `icon-rotation-alignment` is `map`. Glyphs are placed with the widths of Noto Sans; in other fonts, letters may sit a little apart or close together. ([#39](https://github.com/versatiles-org/versatiles-svg-renderer/issues/39))
- **Collision detection for labels and icons.** Every label and icon was drawn, so crowded label layers overlapped. They are now placed as in MapLibre GL JS: from the top layer down, by `symbol-sort-key` within a layer, and a label or icon that would overlap one placed before is left out. `text-allow-overlap`, `icon-allow-overlap`, `*-ignore-placement`, `*-optional` and `*-padding` are supported. Text is measured with the advance widths of Noto Sans (the font of the VersaTiles styles, built into the package), so both renderers keep the same labels. ([#38](https://github.com/versatiles-org/versatiles-svg-renderer/issues/38))
- **`fill-pattern` and `background-pattern`.** Layers with a pattern were drawn in their plain color. They now repeat the sprite image at its display size, anchored to the world, as in MapLibre GL JS; `fill-opacity` applies, a missing image draws nothing, and without a `fill-outline-color` the outline is drawn with the pattern too. The sprite is now also loaded without `renderLabels` when a visible layer uses a pattern. The VersaTiles styles use it for danger areas, prisons and construction sites. At fractional zoom levels a pattern is scaled with the map from the zoom level's integer part, as MapLibre scales it. ([#44](https://github.com/versatiles-org/versatiles-svg-renderer/issues/44))
- **`text-transform`, sort keys and `circle-stroke-opacity`.** Labels follow `text-transform` (`uppercase`, `lowercase`). Features of a layer are drawn in the order of their `fill-sort-key`, `line-sort-key`, `circle-sort-key` or `symbol-sort-key`, lowest first, as in MapLibre GL JS. Circles take `circle-stroke-opacity`. ([#41](https://github.com/versatiles-org/versatiles-svg-renderer/issues/41), [#54](https://github.com/versatiles-org/versatiles-svg-renderer/issues/54))
- **Global state.** `global-state` expressions evaluated to nothing. They now read the defaults of the style's `state`, which the new `globalState` option overrides, like `map.setGlobalStateProperty()` in MapLibre GL JS: in paint and layout properties, filters and `visibility`. The type is exported as `GlobalState`. The SVG export control passes the map's current global state (MapLibre GL JS 5.6 or later). ([#55](https://github.com/versatiles-org/versatiles-svg-renderer/issues/55))
- **`line-gap-width`.** A line with a gap (typically a road casing) was drawn as one solid line. It is now drawn as in MapLibre GL JS: two lines of `line-width`, one on each side of the gap, joined around the line's ends by round and square caps. Round joins follow MapLibre's geometry too: the outer side of a turn is rounded, and at a turn of more than 120° both sides curve around the corner, crossing each other inside it, as MapLibre draws them. `line-round-limit` is supported. ([#53](https://github.com/versatiles-org/versatiles-svg-renderer/issues/53))
- **The style's `center` and `zoom` as the default view.** A view without `lon`, `lat` or `zoom` showed `0, 0` at zoom 2. As in MapLibre GL JS, the style's own `center` and `zoom` now fill in what the view does not set; `0, 0` and zoom 2 remain the fallback for styles without them. This applies to rendering and to `project`/`unproject`. ([#56](https://github.com/versatiles-org/versatiles-svg-renderer/issues/56))
- **Tile source options: `scheme`, `minzoom`, `bounds` and several tile URLs.** Vector and raster sources used only their first tile URL and `maxzoom`. Tiles are now requested as in MapLibre GL JS: spread over all URLs in `tiles` (by the tile's position), with y counted from the south for `scheme: "tms"`, and not at all below `minzoom` or outside `bounds`. These values may also come from a TileJSON document. ([#51](https://github.com/versatiles-org/versatiles-svg-renderer/issues/51))
- **GeoJSON data from a URL.** A GeoJSON source whose `data` is a URL drew nothing. The renderer now loads it with the `fetch` option, once per `SVGMapRenderer` or `PNGMapRenderer`, like a TileJSON document; data that cannot be loaded is reported through `onWarning`, and the next render tries again. ([#52](https://github.com/versatiles-org/versatiles-svg-renderer/issues/52))
- **Warnings about what is not drawn.** Parts of a style the renderer does not draw were skipped silently. They are now reported through the new `onWarning` option (`(message: string) => void`, default `console.warn`), once per renderer: unsupported layer types (`fill-extrusion`, `heatmap`, `hillshade`, `color-relief`), unsupported layer properties (e.g. `fill-pattern`, `symbol-placement: "line"`), unsupported sources (`raster-dem`, `video`), TileJSON documents and GeoJSON data that could not be loaded, and `terrain`. Properties without an effect on a flat, north-up map are not reported, nor are symbol layers unless `renderLabels` is set. The SVG export control lists the warnings below its preview, and also warns when the map is tilted, since the SVG is always flat. ([#58](https://github.com/versatiles-org/versatiles-svg-renderer/issues/58))
...
- **Direction of GeoJSON polygon rings.** The GeoJSON source wound a polygon's outer ring counter-clockwise on screen, the opposite of geojson-vt in MapLibre GL JS and of vector tiles, so a `line-offset` on a GeoJSON polygon's outline moved it to the wrong side. Outer rings now run clockwise and holes counter-clockwise, as in MapLibre.
- **Labels at the vertices of GeoJSON lines and polygons.** A symbol layer on a GeoJSON source placed a label at every vertex of its lines and polygons, besides the ones along the line or inside the polygon. Only circle layers draw at vertices, as in MapLibre GL JS.
- **`icon-rotate` with `icon-offset`.** An icon turned around its point plus its offset. As in MapLibre GL JS, it now turns around its point, offset included.
...
- **`project(view, [lon, lat])` and `unproject(view, [x, y])`** on `SVGMapRenderer` and `PNGMapRenderer`: where a coordinate lands in the image of a view, in the units of `width` and `height`, to place your own drawing on an SVG or a canvas. `undefined` for a point on the far side of the globe. `unproject(view, [x, y])` is the opposite: the coordinate shown at a position in the image, or `undefined` where the image shows no map.
