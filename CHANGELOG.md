...
- **`project(view, [lon, lat])`** on `SVGMapRenderer` and `PNGMapRenderer`: where a coordinate lands in the image of a view, in the units of `width` and `height`, to place your own drawing on an SVG or a canvas. `undefined` for a point on the far side of the globe.
